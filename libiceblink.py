...
class ICE40Board(object):
    ...
    class __ICE40SPIPort(object): #class
        def __init__(self, dev, portno):
            self.dev = dev # nested class
        ...
        def open(self): #open
            assert not self.__is_open
            pl = self.dev.checked_cmd(0x03, 0x06, "SPIOpen", [self.__portno])
            assert len(pl) == 0
            self.__is_open = True
    ...
    def __init__(self): #init
        ...
        print(self.ep_cmdout, self.ep_cmdin)
    ...
    def checked_cmd(self, cmd, subcmd, name, payload=[], ressize=16, show=False, #check
                   noret=False):
    ...
    def cmd(self, cmd, subcmd, payload, ressize=16, show=False):  #cmd
        res = self.cmd_i(
